...
10. **Push + PR immediately, then independent review** — when a change is complete and the local gates are green, push the feature branch and open its PR right away, without waiting for prompting (`main` stays untouchable — rule 2). After the PR is opened, a **fresh reviewer agent with no shared context with the author** reviews it — evaluates the code and leaves its findings as PR comments — and the original author agent then addresses those comments. **Merging remains the user's decision** — an agent never merges a PR on its own initiative. Only destructive rewrites of already-pushed history still need explicit approval.
11. **Released Liquibase changesets are immutable** — qnop has published releases, so a shipped changeset is never edited (checksums are validated on every deployed instance). Every schema change is a NEW `NNNN-*.yaml` migration; the old "fold into the existing changeset" practice is retired.
