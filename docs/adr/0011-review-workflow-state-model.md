...
## Amendment (2026-07-23, issue #576 — auto-archive of long-closed reviews)

A `FINALIZED`/`CANCELLED` review is a record, not a working object, yet it stays in the review lists forever. This amendment moves long-closed reviews out of the active lists automatically, without inventing a new workflow state:

- **Archiving is an orthogonal retention flag, not a workflow state.** `workflow_state` stays `FINALIZED`/`CANCELLED`; a nullable `document.archived_at` marks a review as archived. This deliberately preserves the **terminal outcome** (the UI reads "Archived · was Finalized"), and — because the existing closed-record guards already key off `FINALIZED`/`CANCELLED` (the check a new `WorkflowState.isClosed()` helper names) — an archived review is already read-only with no new guard. Modelling it as a real `ARCHIVED` state was rejected: it would overwrite (and so lose) the terminal outcome, force a shadow column to recover it, and add edges + a third closed-state to three guards for no gain. A nullable `document.closed_at`, set once on the terminal transition, gives the sweep a cheap "closed since" predicate (the terminal instant otherwise lives only in the `workflow.transition` audit trail — too expensive per row); existing rows are backfilled from `updated_at`.
- **System-driven, with an operator gate and a manual path.** A `reviewArchive` scheduler job (issue #524, ADR-0045) archives reviews closed longer than `review.archive_after_days` (default 90; **0 disables**), ShedLock-guarded (ADR-0029), off-peak, dry-run-capable — enable/disable, dry-run and run-now on `/admin/scheduler`. An owner or admin may also archive a closed review now, or unarchive one (the #563 moderation direction; the flag makes unarchive trivial and keeps the record fully recoverable). Each archived review gets a per-document `review.archived` audit event (system-driven → null actor, "System" in the trail; manual → the acting user); unarchive audits `review.unarchived`.
- **Archived reviews leave the default lists but stay reachable.** The reviews overview and the dashboard aggregates exclude archived reviews by default (a server-side `archived` filter on `GET /documents`); an explicit "Archived" view returns them. The review itself — versions, annotations, threads, audit — remains a fully readable, immutable record exactly as before.
