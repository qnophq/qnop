...
## Amendment (2026-08-31, issue #601): the accept gate derives from the registered extractors

The upload accept gate was hard-coded to PDF-and-DOCX, so a registered `DocumentExtractor` for another format was rejected before it ever ran — breaking this ADR's "adding a format = implementing one `DocumentExtractor`" promise. Now: a sniffed type passes the gate when it is PDF, DOCX-with-converter, or **claimed by a registered extractor** (`DocumentRenditionService.supports`); such a type is extracted directly (no office conversion). The SPI grows `default Set<String> mediaTypes()` (semver minor) as the enumerable face of the claim, surfaced to clients via `GET /config` `supportedMediaTypes` (and mapped onto the `SupportedFormat` enum where its vocabulary covers the type), from which the upload UI derives its file filter. Sniffing stays bytes-only and grew the common image magics — recognition and acceptance remain separate questions: an unclaimed recognized type is still a 415.
