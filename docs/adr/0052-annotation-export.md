...
The export reads the annotation's *unabridged* opening comment, not the excerpt the annotation list carries. `AnnotationView.firstComment` is capped at 300 characters for a list row; building the export on it truncated every finding, and a cut landing inside an image reference left a broken link rather than a shortened sentence. One batched query over the already-authorized views supplies the full text.
...
In the spreadsheet the logo sits in the top-left corner of *every* sheet, at half the rendition's size — halved in the renderer rather than in the rendition, because one rendition serves both formats and Word scales it down to about 90pt where the extra pixels are print sharpness — the anchor is computed from the picture, never the picture squeezed into the anchor. It gets a band of its own above the header, whose height comes from the picture: floating it over the grid was the literal reading of "on top" and the wrong one, because it hid the cells underneath. With no logo the sheet keeps the layout that shipped with #547.
...
In the spreadsheet the link cannot sit in the prose at all: Excel attaches a hyperlink to a whole cell, never to a word inside one, so each upload needs a cell of its own. Putting those cells on the annotation's row was tried and abandoned: a review whose busiest annotation carries five files gives *every* row five columns, four of them empty. They live on an `Attachments` sheet instead — task key, filename as the link, type, size — which holds any number without deforming the grid and sorts and filters like the others. The prose keeps a `[name]` marker showing where each was mentioned. Images are listed too, since Word embeds them and a spreadsheet cannot, and `[screenshot.png]` with no way to open it helps nobody.
