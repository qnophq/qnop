...
In the spreadsheet the link cannot sit in the prose at all: Excel attaches a hyperlink to a whole cell, never to a word inside one. Each upload therefore gets a cell of its own, appended after the fixed columns on the row it belongs to — the annotation's on the annotation row, a comment's on the comment row — while the prose keeps a `[name]` marker showing where it was mentioned. The number of those columns is what the busiest row actually uses: one attachment does not cost five empty columns, and five do not lose four. Images get a cell too, since Word embeds them and a spreadsheet cannot, and `[screenshot.png]` with no way to open it helps nobody.
