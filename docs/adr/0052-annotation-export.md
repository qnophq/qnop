...
In the spreadsheet the links cannot sit in the prose at all: Excel attaches a hyperlink to a whole cell, never to a word inside one. Every referenced upload therefore gets a row on an `Attachments` sheet — task key, filename as the link, type, size — while the prose keeps a `[name]` marker showing where it was mentioned. Images are listed there too: Word embeds them and a spreadsheet cannot, so without the row a reader would see `[screenshot.png]` with no way to open it.
