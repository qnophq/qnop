...
Consequently a machine principal is *authenticated but nearly powerless* in the community core: it passes `.authenticated()`, and every user-actor and role-gated path refuses it. That is deliberate. **Authorisation for machine access is the extension's business** (qnop-ee#19's scopes decide which of its own endpoints — or later, which opened read paths — a key may call). Opening individual community read paths to machine actors is the expected re-cut (ADR-0049), and it rides on the shared principal work with #684, not on this ADR.
