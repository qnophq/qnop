...
- **Implemented by `qnop-app`**: controllers depend on `qnop-api` and map between its DTOs and application types.
...
- Clear separation of contract (`qnop-api`, published) vs. implementation (`qnop-app`, internal); the wire format never leaks into the domain.
- One more module + the discipline of treating the API as a stable surface (versioning, deprecation policy).

## Alternatives considered

- **Keep DTOs + OpenAPI inside `qnop-app`** — rejected: ties the consumer-facing contract to the server runtime and makes a clean published/generated client awkward. Acceptable only for an internal-frontend-only API (not our case).
- **A second generic "REST" module beside `qnop-app`** — rejected: would split the same implementation concern and reintroduce naming confusion; `qnop-app` already is the REST adapter.
