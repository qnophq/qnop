...
- **Configuration via environment.** `application.yml` builds the datasource from the **same `POSTGRES_*` variables `docker-compose.yml` uses** (`POSTGRES_HOST`/`POSTGRES_PORT`/`POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD`), so a single `.env` drives both the DB container and the app (the official Postgres image mandates the `POSTGRES_*` names, so the app aligns to them rather than the reverse). Local-dev defaults match `docker-compose.yml`; production may instead set the standard `SPRING_DATASOURCE_*` keys, which override.
