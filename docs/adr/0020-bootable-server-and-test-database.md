...
- **Tests run against a real PostgreSQL via Testcontainers** (`@ServiceConnection`, image `postgres:18` to match local infra — `docker-compose.yml`/`docker-compose.smoke.yml`; bumped from `postgres:17` in #199 after the compose image moved to 18, restoring the test/infra parity). A context test boots the app, lets Liquibase apply the baseline changelog, and asserts the actuator health endpoint reports `UP`.
