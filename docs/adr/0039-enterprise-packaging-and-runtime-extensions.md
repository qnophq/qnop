...
**2026-08-31 — second slot family (issue #600).** The message-row **actions** and **badge** slots live in `qnop-ui/src/extensions/` inside the host bundle; the slot types are the seed of `qnop-ui-spi` and move into that package when the runtime loader lands.
