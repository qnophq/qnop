...
- **`qnop-api` becomes a container** holding only the spec, with two submodules generated from it:
  ...
  - **`qnop-api:qnop-api-endpoint`** — the `spring` generator in apis-only mode (`globalProperties = ["apis", "supportingFiles"]`, `interfaceOnly=true`). Emits Spring MVC interfaces into `io.qnop.api.v1.endpoint` that import the DTOs from `qnop-api-model`. **Depends on Spring by design.**
