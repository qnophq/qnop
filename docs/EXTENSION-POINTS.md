...
- **Advertisement (issue #601, ADR-0032 amendment):** `default Set<String> mediaTypes()` is the enumerable face of the claim — it makes the type pass the upload accept gate, surfaces in `GET /config` `supportedMediaTypes` (from which the upload UI derives its file filter), and must agree with `supports`. A claimed type is extracted directly; the office conversion stays DOCX-only.
...
- **Core keeps:** the catalogue (the internal→published mapping is exhaustive over the sealed `ReviewEvent` hierarchy, so a new internal event is a compile error until someone decides its published fate), the isolation guarantees, and **anonymity**: in an anonymous review (ADR-0038) no identity crosses the seam — `actorId` is nulled and identity-bearing attributes (`userId`, `teamId`) are stripped before any listener hears the event. **No ordering guarantee** — events may arrive concurrently or out of order. `ReviewNotificationSink` stays internal and recipient-shaped beside this event-shaped seam.
