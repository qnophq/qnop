...
/**
 * Pure, DB-free derivation of team slugs from team names (issue #470), mirroring {@link UserSlugs}.
 * The output always satisfies the {@code ck_team_slug} shape (kebab-case, 3-64 characters) and is
 * never UUID-shaped, since {@code /my-teams/{ref}} resolves UUID-shaped segments as team ids.
 * Uniqueness is NOT handled here — {@link TeamSlugService} probes {@link #candidate(String, int)}
 * against the database.
 */
public final class TeamSlugs {

  static final int MAX_LENGTH = 64;
  private static final int MIN_LENGTH = 3;

  /** Base for names that slugify to nothing (e.g. purely non-Latin symbols). */
  private static final String FALLBACK = "team";
  ...
  /**
   * Derives the base slug for a team name: diacritics folded (NFKD), lowercased, every
   * non-alphanumeric run collapsed to a single hyphen. Too-short (and, defensively, UUID-shaped)
   * results are suffixed with {@code -team}; empty results fall back to {@code team}.
   */
