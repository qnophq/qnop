...
public interface DocumentExtractor {
   ...
   * accept gate. {@link #supports} stays the authoritative per-upload predicate; this set is the
   * enumerable face of the same claim, so the two must agree. The default keeps existing
   * implementations compiling (semver minor, ADR-0032 amendment) — but an extractor that returns an
   * empty set is invisible to the gate and to clients.
   */
